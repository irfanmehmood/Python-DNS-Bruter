...
def generate_subdomains():
    ...
    for r, d, f in os.walk(path):
        for file in f:
            if '.txt' in file and 'subdomain-dictionary.txt' not in file:
                print ("[" + file + "] MERGED")
...
def run_dnscan(domain, recursive=False, torred=True):
    ...
    data_line = format_results(output_file)

    # Creat a new scan row and gets its ID
    scan_id = db.scan_add_result(domain, 'dnscan', command , data_line, output_file, datetime.datetime.utcnow())
